...
//!
//! Цикл X «Обрушение» (v0.60.0): нелинейная спектральная гидродинамика —
//! то, что OpenAI решала тераваттами брут-форсом. 2-й порядок Стокса:
//! h₂ = (kA²/2)·cos2θ (гребень острее, ложбина площе) + сдвиг частоты
//! ω·(1+(kA)²/2) — в том же целочисленном счётчике фаз, без дрейфа.
//! Снос сверхкрутых гребней по Мичеллу (kA > s_b → спиллинг, срез до
//! 0.9·порога, энергия события — бухгалтерски: когерентный вихрь + тепло)
//! и белые барашки по Бофорту (наблюдательный триггер ≈ 9 м/с). Вихрь
//! обрушения — Ламб–Озеен на фикс-точке: вязкое расплывание ядра
//! r² = r₀²+8νt, лагранжева адвекция полем волн (дрейф Стокса возникает
//! сам), растяжение деформацией dΓ/dt = Γ·∂u/∂x с клипом. Поверхностный
//! след — горб напора Δh = Γ²/(4π²gr²)·e^(−d²/2r²); `flow_full_at` —
//! первое РОТАЦИОННОЕ поле движка (циркуляция = Γ·(1−e^(−d²/r²)),
//! теорема Стокса — тест меряет).
