...
updated_utc: 2026-08-29T21:55:00Z
repo: poler-engine
branch: main
...
current_task: v0.18.0 License Gate — РЕАЛИЗОВАН И ПРОВЕРЕН E2E: офлайн-лицензии ed25519 (формат PO1), тиры community/pro/enterprise + trial 14 дн., Community-квота 50 интеграционных операций за скользящие 24 ч, grace 7 дн. после истечения; локальный поиск/резонанс/AIDDE/TUI — БЕЗ лицензии всегда
current_task_note: src/license/mod.rs (~800 строк, 20 тестов: base64url все 256 байт + reject стандартного алфавита/паддинга, чужая подпись, подмена payload, будущая дата выдачи, бессрочный pro запрещён, grace, скользящее окно квот, civil-даты Хиннанта); лицензия владельца активирована (pro до 2036-08-26); license-tool/ — отдельный крейт выпуска (keygen + issue, /dev/urandom, приватный seed 0600 в ~/.config/poler-engine/license-signing-key, в git НЕ попадает, добавлен в secret-scan); гейты: --google-gmail/--google-drive/--nlm-* + shell nlm + MCP poler_gmail/poler_drive; --license/--license-import CLI; env POLER_LICENSE_KEY/POLER_LICENSE_FILE; прозрачность: --google-auth печатает что получает движок (NotebookLM — НЕ OAuth-скоуп, синк через профиль браузера); 687 тестов зелёные; live: gmail под pro прошёл
next_task: выбор площадки продаж (MoR: LemonSqueezy/Paddle 5%+50¢ — они продавец, налоги/НДС их; Steam: $100/app + W-8BEN + 30% и уклон в игры — НЕ рекомендуется первым шагом); перенос репо в org poler-engine-org (создана 2026-08-29, free, 0 репо — перенос сохранит историю и редиректы, ждать явного добра владельца); Privacy Policy в отдельном публичном репо org-а для верификации Google; при платящих юзерах — CASA Tier 2 ($540)
